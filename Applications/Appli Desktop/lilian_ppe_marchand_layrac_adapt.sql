...
-- Listage des données de la table lilian_ppe_marchand_layrac_adapt.avisclient : 1 rows
...
-- Listage des données de la table lilian_ppe_marchand_layrac_adapt.categorie : ~3 rows (environ)
...
-- Listage des données de la table lilian_ppe_marchand_layrac_adapt.client : ~101 rows (environ)
...
-- Listage des données de la table lilian_ppe_marchand_layrac_adapt.commande : ~100 rows (environ)
...
-- Listage de la structure de l'évènement lilian_ppe_marchand_layrac_adapt. GestionStats
DELIMITER //
CREATE EVENT `GestionStats` ON SCHEDULE EVERY 10 SECOND STARTS '2022-04-01 16:00:00' ENDS '2022-04-23 15:58:30' ON COMPLETION PRESERVE ENABLE DO BEGIN

DECLARE commande INT;
DECLARE client INT;
DECLARE produit INT;

SET commande = (SELECT COUNT(idCommande) FROM commande);
SET client = (SELECT COUNT(idClient) FROM client);
SET produit = (SELECT COUNT(idProduit) FROM produit);

INSERT INTO statistiques (NBCommandes, NBClients, NBProduits, DateStatistiques)
VALUES (commande, client, produit, NOW());
...
-- Listage de la structure de la table lilian_ppe_marchand_layrac_adapt. statistiques
CREATE TABLE IF NOT EXISTS `statistiques` (
  `NBCommandes` int(11) DEFAULT NULL,
  `NBClients` int(11) DEFAULT NULL,
  `NBProduits` int(11) DEFAULT NULL,
  `DateStatistiques` datetime DEFAULT NULL,
  KEY `FK_commande` (`NBCommandes`),
  KEY `FK_client` (`NBClients`),
  KEY `FK_produit` (`NBProduits`)
) ENGINE=MyISAM DEFAULT CHARSET=latin1;

-- Listage des données de la table lilian_ppe_marchand_layrac_adapt.statistiques : 1 rows
/*!40000 ALTER TABLE `statistiques` DISABLE KEYS */;
INSERT INTO `statistiques` (`NBCommandes`, `NBClients`, `NBProduits`, `DateStatistiques`) VALUES
	(100, 101, 6, '2022-04-04 00:28:50');
...
-- Listage de la structure de l'évènement lilian_ppe_marchand_layrac_adapt. ViderStats
DELIMITER //
CREATE EVENT `ViderStats` ON SCHEDULE EVERY 10 SECOND STARTS '2022-04-01 16:00:00' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
